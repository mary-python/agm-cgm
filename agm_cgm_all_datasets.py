...
def runLoop(dataIndex, dim, num, newImages, labels, GS):
    ...
    for fi in range(6):
        ...
        if fi == 0:
            ...
            mseDispETableAC = np.round(np.divide(mseDispETableA, mseDispETableC), 6)
            ...
            mseQETTableA = np.round(np.divide(mseQETableA, mseQTTableA), 7)
            mseQETTableC = np.round(np.divide(mseQETableC, mseQTTableC), 7)
